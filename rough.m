...
clc
filename1='5f1.csv';
[data1,A1]=initiate3(filename1);
filename2='5s1.csv';
[data2,A2]=initiate3(filename2);
% imshow(A1)
...
A2=flip(A2,2);
[X2,Y2,x2,y2]=mysmooth(A2);
[~,loc3]=findpeaks(-X2(minloc(y2):maxloc(y2)));
...
%
d2=sqrt((ps(1,1)-ps(2,1)).^2+(ps(2,2)-ps(1,2)).^2);
[t1,t2]=pixeltobody(filename1,filename2);
d=pi*(d1*t1+d2*t2)/2;
...
hold on
plot(p(:,1),p(:,2),'r*');
% plot(x1(gg),y1(gg),'b*','linewidth',2);
% plot(x1(loc(1)),y1(loc(1)),'b*','linewidth',2);
% plot(x1,y1,'b-','linewidth',2);
% plot(X1,Y1,'b-','linewidth',2);
% plot(x1(loc2(end)+minloc(y1)),y1(loc2(end)+minloc(y1)),'b*','linewidth',2);
gg=1;
figure;
imshow(A2);
hold on
% plot(ps(2,1),ps(2,2),'r*');
% plot(ps(1,1),ps(1,2),'r*');
plot(ps(:,1),ps(:,2),'r*');
% plot(x2,y2,'b-','linewidth',2);
% plot(X2,Y2,'b-','linewidth',2);
% plot(x2(gg),y2(gg),'b*','linewidth',2);